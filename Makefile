...
build: node_modules ## Compile TypeScript to dist/
	@echo "=== build ==="
	npx tsup cli/index.ts --format esm --dts --outDir dist
...
lint-format: node_modules ## Lint + format source (auto-fix)
	...
	npx @biomejs/biome lint --write cli 2>&1 | tee artifacts/lint-format/lint-report.txt
	npx @biomejs/biome format --write cli 2>&1 | tee artifacts/lint-format/format-report.txt
	@echo ""
	@echo "Artifacts:"
...
lint-check: node_modules ## Lint + format check only (no writes)
	npx @biomejs/biome lint cli
	npx @biomejs/biome format cli
...
release-prep: node_modules ## Prepare release (VERSION=x.y.z)
	...
	@# Lint
	npx @biomejs/biome lint cli && echo "- [x] Lint: clean" >> artifacts/release-prep/checklist.md || echo "- [ ] Lint: FAILED" >> artifacts/release-prep/checklist.md
	@# Format
	npx @biomejs/biome format cli && echo "- [x] Format: clean" >> artifacts/release-prep/checklist.md || echo "- [ ] Format: FAILED" >> artifacts/release-prep/checklist.md
	@# Build
	npx tsup cli/index.ts --format esm --dts --outDir dist
...
ens-resolve: node_modules ## Resolve ENS name (NAME=vitalik.eth NETWORK=mainnet)
	...
	npx tsx cli/index.ts resolve $(NAME) --network $(NETWORK) 2>&1 | tee artifacts/ens-resolve/resolve-log.txt
	@echo ""
...
ens-register: node_modules ## Register ENS name (NAME=myname NETWORK=sepolia)
	...
	npx tsx cli/index.ts register $(NAME) --duration $(DURATION) --network $(NETWORK) 2>&1 | tee artifacts/ens-register/register-log.txt
	@echo ""
