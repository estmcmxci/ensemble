...
| `name` | `string` | ENS name to auto-load on mount (e.g. `"vitalik.eth"`). Fetches that profile regardless of wallet connection |
| `profile` | `ENSProfile` | Pre-resolved ENS profile data. Skips the initial fetch when provided |
...
| `chatApiUrl` | `string` | ChatKit API URL — enables the chat panel when provided |
| `chatDomainKey` | `string` | ChatKit domain key — required alongside `chatApiUrl` |
| `chatTheme` | `Record<string, unknown>` | ChatKit theme object |

### Using `name` and `profile`

Pass `name` to auto-load a specific ENS profile on mount, without requiring a wallet connection:

```tsx
<ENSemble apiUrl="https://your-api.workers.dev" name="vitalik.eth" />
```

Pass `profile` to skip the initial fetch entirely — useful when your app has already resolved the ENS data:

```tsx
<ENSemble apiUrl="https://your-api.workers.dev" profile={preResolvedProfile} />
```

Both props are optional. When neither is provided, the component fetches the connected wallet's profile automatically.
...
### `useENSProfile(initialName?, externalProfile?)`

Profile and name list fetching. Accepts optional params to pre-populate data:

```tsx
// Wallet-based (default)
const { profile, nameList, isLoading, refresh, selectName } = useENSProfile();

// Name-based — fetches this profile on mount
const { profile } = useENSProfile('vitalik.eth');

// Pre-resolved — uses provided data, skips fetch
const { profile } = useENSProfile(undefined, existingProfile);
```
