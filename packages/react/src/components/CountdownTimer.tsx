...
export function CountdownTimer({ waitSeconds, onComplete }: CountdownTimerProps) {
  ...
  return (
    <div className="ens-timer">
      <div className="ens-timer__row">
        <span className="ens-timer__label">
          {isDone ? 'Commit-reveal wait complete' : 'Waiting for commit-reveal period...'}
        </span>
        <span className={`ens-timer__value ${isDone ? 'ens-timer__value--done' : 'ens-timer__value--active'}`}>
          {minutes}:{seconds.toString().padStart(2, '0')}
        </span>
      </div>
      <div className="ens-timer__bar">
        <div
          className={`ens-timer__fill ${isDone ? 'ens-timer__fill--done' : 'ens-timer__fill--active'}`}
